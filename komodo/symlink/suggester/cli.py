...
from komodo.symlink.suggester.configuration import update as config_update
...
def suggest_symlink_configuration(args, repo):
    ...
    target_branch = "{}/{}".format(args.release, args.mode)

    from_sha = repo.get_branch(args.git_ref).commit.sha
    repo.create_git_ref(ref="refs/heads/" + target_branch, sha=from_sha)

    msg = "Update {} symlinks for {}".format(args.mode, args.release)
    repo.update_file(
